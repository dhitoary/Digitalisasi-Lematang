...
(function () {
    ...
    mutations.forEach((mutation) => {
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === 1) {
          ...
          if (
            isSuspicious ||
            (hasScript && !node.src.includes("desalematang")) ||
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      if (
        ...
        if (
          !confirm("Anda akan meninggalkan website Desa Lematang. Lanjutkan?")
        ) {
          e.preventDefault();
          return false;
        }
  ...
  function checkIntegrity() {
    ...
    const navbar =
      document.querySelector("nav") || document.querySelector('[id*="nav"]');
    if (!navbar) {
